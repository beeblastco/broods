...
- In-process policy check, not the OPA sidecar. API authorization is a per-request allow/deny over a small closed action set. The rego sidecar earns its cost for in-run tool decisions with conditions; a table-driven check is enough here and adds no network hop. If the two ever need to merge, the shared document shape makes that a refactor, not a rewrite.
