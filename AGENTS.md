...
- `apps/matrix-forwarder` (`@broods/matrix-forwarder`) runs the Matrix `/sync` loops. Matrix has no webhooks, so it syncs, decrypts and forwards room messages to core, and sends core's replies back through the account's end-to-end keys. one sync loop per access token, crypto store on a persistent volume, single replica.
- `apps/matrix-forwarder` (`@broods/matrix-forwarder`) runs the Matrix `/sync` long-polls and holds each account's E2EE keys. Matrix has no webhooks, so it POSTs decrypted room messages to the channel webhook, and core sends room events (replies, reactions, typing) back through it because only it can encrypt them. one long-poll per access token.
