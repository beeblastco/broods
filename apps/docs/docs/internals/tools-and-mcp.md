...
7. `ask_questions` when the session has a delivery to resume on (a channel or WebSocket turn) and the run is not cron-fired. Subagent sessions carry no delivery, so they never get it.
...
Core is the MCP client, spec 2026-07-28, stateless Streamable HTTP only. At agent registration it connects to each enabled server, lists tools (cached for the listing's own `ttlMs`), and registers each as `<server>__<tool>`. `tools/call` is one POST per call with no session.
...
- Add approval through `needsApproval` ([AI SDK tool approval](https://ai-sdk.dev/docs/ai-sdk-core/tools-and-tool-calling#tool-execution-approval)), never by asking inside the tool.
