...
`harnessReservationKey` in `src/harness/ai-sdk-harness/session.ts` picks the base key. A Pi conversation reserves on the agent-level key of its first sandbox, so every conversation of that agent shares one machine, each in its own `pi-<sessionId>` work folder. A subagent task with `isolated: true`, and every bridge adapter, reserves on the conversation key instead, and that reservation gets a one-day idle window (`releaseAfterIdleSeconds` on the control plane, `ttlSeconds` on the Convex reservation) instead of seven days. The key is stored on `runtimeHarnessSessions`, so a conversation always resumes on the machine it started on; rows without one ran on the conversation key. A session ending on a shared machine neither suspends nor deletes it; the provider's idle stop does.

Every run with a harness, or a bash-only agent with a persistent default sandbox, gets its machine's live state at the end of `<environment>`, after the cached prefix: size and the other runs holding it right now, plus, on a harness turn, the guest's CPU load, memory and disk from one probe exec (capped at 3s) once the session holds the machine. A bash-only turn does no I/O for it, so it never waits on the provider. The first-time onCreate step takes a lock in the guest, so runs that acquire a new shared machine together run it once. `src/harness/sandbox/live-status.ts` keeps that occupancy in memory, which is correct because core runs one replica.
...
The upstream URL and provider credential travel inside the AES-256-GCM sealed ticket (`src/shared/terminal-ticket.ts`), so the browser only holds an opaque short-lived token. The gateway accepts each ticket once, so a copy seen in transit cannot open a second shell. It records spent tickets by SHA-256 in the `TERMINAL_TICKETS_SPENT` JetStream KV bucket, whose keys expire with the ticket TTL, so the record holds across replicas and restarts. If NATS cannot be reached the upgrade answers 502 rather than skip the check. Workdir tickets carry the sandbox's workdir API key as a bearer header: `options.apiKey`, required with a custom `options.workdirUrl`, else `WORKDIR_API_KEY` for the platform node. MicroVM tickets carry a `CreateMicrovmShellAuthToken` JWE, valid 30 minutes, in `X-aws-proxy-auth` for the VM's native shell. Opening a terminal resumes a suspended instance first.
