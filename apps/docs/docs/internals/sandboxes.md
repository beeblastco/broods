...
Every run with a harness, or a bash-only agent with a persistent default sandbox, gets its machine's live state at the end of `<environment>`, after the cached prefix: size, state, the guest's CPU load, memory and disk (one probe exec when a harness acquires the machine), and the other runs holding it right now. `src/harness/sandbox/live-status.ts` keeps that occupancy in memory, which is correct because core runs one replica.
