...
- **Non-fatal.** A hook that throws or times out is logged and skipped; the agent run continues with unmutated state. Hooks are wall-clock bounded, and a skipped hook denies nothing, so a hook deny is best effort. Anything that must hold belongs in [an enforced policy](resources.md#policies), which fails closed.
