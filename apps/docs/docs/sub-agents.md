...
- A child carries the parent's effective `policies` and `denyTools` on top of its own, so a channel record that withholds `bash` from the parent withholds it from every child too.
