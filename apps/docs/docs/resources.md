---
id: resources
title: Resource Configuration
---

# Resource Configuration

BeeBlast uses a **code-first** configuration model. You define agents, workspaces, sandboxes, skills, tools, cron jobs, and channels as typed TypeScript resources inside a `filthypanty/` folder. The CLI compiles these into a manifest, syncs them to the cloud, and generates typed runtime references.

## Project Layout

```text
my-project/
  filthypanty/
    index.ts           # Resource definitions
    _generated/        # Auto-generated by filthy-panty dev/deploy
      api.ts           # Typed agent/resource references
      ids.ts           # Deployed IDs
    greeting-skill/    # Skill bundle (optional)
      SKILL.md
    tools/             # Custom tool sources (optional)
      my-tool.ts
  .env.local           # Local secrets (never commit)
```

## `env` — Deferred Secrets

Never bake secrets into your resource files. Use `env.NAME` to create a **deferred reference** that resolves on the server at runtime:

```ts
import { defineAgent, env } from "filthy-panty";

export const myAgent = defineAgent({
  name: "my-agent",
  config: {
    provider: {
      openai: { apiKey: env.OPENAI_API_KEY },
    },
  },
});
```

Both forms are equivalent:

```ts
env.OPENAI_API_KEY      // property access
env("OPENAI_API_KEY")   // call form
```

Set the value with the CLI:

```bash
filthy-panty env set OPENAI_API_KEY
```

Or let `filthy-panty dev` auto-push it from your local `.env.local`.

> `env` is **not** `process.env`. Using `process.env.OPENAI_API_KEY` would bake your local value into the deployed config. Always use `env.NAME` for anything that should stay server-side.

## Agents

`defineAgent` is the core resource. It holds model, provider, tools, channels, workspaces, skills, subagents, and hooks.

```ts
import { defineAgent, defineSandbox, defineWorkspace, env } from "filthy-panty";

export const myAgent = defineAgent({
  name: "my-agent",
  description: "A general-purpose assistant", // visible to parent agents
  config: {
    provider: {
      google: { apiKey: env.GOOGLE_API_KEY },
    },
    model: {
      provider: "google",
      modelId: "gemini-3-flash",
      temperature: 0.7,
    },
    agent: {
      system: "You are a helpful assistant.",
      maxTurn: 10,
    },
    publicAccess: true, // enable public SSE/WebSocket endpoint
  },
});
```

### Supported Providers

| Provider | Config key | Required fields |
| --- | --- | --- |
| Google | `google` | `apiKey` |
| OpenAI | `openai` | `apiKey` |
| Bedrock | `bedrock` | `region`, `apiKey` |
| Gateway | `gateway` | `apiKey` |
| MiniMax | `minimax` | `apiKey` |

### Reasoning / Thinking Tokens

Use standard Vercel AI SDK `providerOptions` in `config.model`:

```ts
model: {
  provider: "openai",
  modelId: "o3",
  providerOptions: {
    openai: {
      reasoningEffort: "high",
      reasoningSummary: "auto",
    },
  },
},
```

| Provider | Thinking config |
| --- | --- |
| OpenAI | `providerOptions.openai.reasoningEffort` |
| Anthropic | `providerOptions.anthropic.thinking` |
| Google | `providerOptions.google.thinkingConfig` |
| MiniMax | `providerOptions.anthropic.thinking` (Anthropic-compatible) |

### Structured Output

```ts
model: {
  provider: "google",
  modelId: "gemini-3-flash",
  output: {
    type: "object",
    name: "analysis",
    description: "Structured analysis result",
    schema: {
      type: "object",
      properties: {
        summary: { type: "string" },
        confidence: { type: "number" },
      },
      required: ["summary", "confidence"],
    },
  },
},
```

### Tools

Enable built-in and custom tools:

```ts
config: {
  tools: {
    tavilySearch: {
      enabled: true,
      apiKey: env.TAVILY_API_KEY,
      searchDepth: "advanced",
    },
    googleSearch: { enabled: true },
    handoffs: { enabled: true },
  },
},
```

See [External Tools](tools.md) for uploading custom tools.

### Channels

Attach one or more channels to an agent:

```ts
import { defineTelegramChannel } from "filthy-panty";

export const telegram = defineTelegramChannel({
  botToken: env.TELEGRAM_BOT_TOKEN,
  webhookSecret: env.TELEGRAM_WEBHOOK_SECRET,
  allowedChatIds: [123456789],
  streaming: { mode: "progress" },
});

export const myAgent = defineAgent({
  name: "my-agent",
  config: {
    channels: [telegram],
  },
});
```

Each channel type has its own constructor. See [Channels](channels/index.md).

### Workspaces

Reference workspace resources or pass per-workspace sandbox overrides:

```ts
import { defineWorkspace } from "filthy-panty";

export const notes = defineWorkspace({
  name: "notes",
  config: {
    storage: { provider: "s3" },
    harness: { enabled: true },
  },
});

export const myAgent = defineAgent({
  name: "my-agent",
  config: {
    sandbox: mySandbox,
    workspaces: [
      notes,                                          // inherit agent sandbox
      { workspace: notes, sandbox: null },            // read-only
      { workspace: teamWorkspace, sandbox: k8sSandbox }, // per-workspace override
    ],
  },
});
```

### Subagents

```ts
export const research = defineAgent({
  name: "research",
  config: { /* ... */ },
});

export const myAgent = defineAgent({
  name: "my-agent",
  config: {
    subagent: {
      enabled: true,
      allowed: [research],
      context: "inherited",
      mode: "persistent",
    },
  },
});
```

See [Sub Agents](sub-agents.md) for full details.

### Skills

```ts
import { defineSkill } from "filthy-panty";

export const supportFlow = defineSkill({
  name: "support-flow",
  config: { path: "support-flow" },
});

export const myAgent = defineAgent({
  name: "my-agent",
  config: {
    skills: {
      enabled: true,
      allowed: [supportFlow],
    },
  },
});
```

See [Skills](skills.md) for bundle format.

### Hooks

Configure lifecycle webhooks:

```ts
config: {
  hooks: {
    webhooks: [
      {
        enabled: true,
        url: "https://example.com/webhooks",
        secret: env.WEBHOOK_SECRET,
        events: ["agent.started", "agent.finished", "agent.failed"],
      },
    ],
  },
},
```

See [Lifecycle Webhooks](webhook.md).

### Session

Control history pruning and compaction:

```ts
config: {
  session: {
    pruning: { enabled: true },
    compaction: {
      enabled: true,
      maxContextLength: 100_000,
    },
  },
},
```

## Sandboxes

```ts
import { defineSandbox, env } from "filthy-panty";

export const lambdaSandbox = defineSandbox({
  name: "lambda-sandbox",
  config: {
    provider: "lambda",
    network: { mode: "deny-all" },
    permissionMode: "ask",
    timeout: 60,
  },
});

export const k8sSandbox = defineSandbox({
  name: "persistent-k8s",
  config: {
    provider: "kubernetes",
    network: { mode: "allow-all" },
    permissionMode: "bypass",
    persistent: true,
    lifecycle: { idleTimeoutSeconds: 1800 },
    options: { mountAwsS3Buckets: true },
  },
});
```

Supported providers: `lambda`, `e2b`, `daytona`, `kubernetes`, `vercel`.

See [Workspace & Sandbox](workspace/index.md) for the full sandbox model.

## Workspaces

```ts
import { defineWorkspace } from "filthy-panty";

export const notes = defineWorkspace({
  name: "notes",
  config: {
    storage: { provider: "s3" },
    harness: { enabled: true },
  },
});
```

## Skills

```ts
import { defineSkill } from "filthy-panty";

export const supportFlow = defineSkill({
  name: "support-flow",
  config: { path: "support-flow" },
});
```

The `path` is relative to `filthypanty/` and must contain a `SKILL.md` file.

## Tools

```ts
import { defineTool } from "filthy-panty";

export const analyze = defineTool({
  name: "analyze",
  config: {
    path: "tools/analyze.ts",
    description: "Analyze structured data.",
    inputSchema: {
      type: "object",
      properties: {
        data: { type: "array" },
      },
      required: ["data"],
    },
    defaultConfig: {},
  },
});
```

The CLI bundles the TypeScript entrypoint into a self-contained ESM module and uploads it. See [External Tools](tools.md) for the bundle contract.

## Cron Jobs

```ts
import { defineCron } from "filthy-panty";

export const dailyDigest = defineCron({
  name: "daily-digest",
  config: {
    agent: myAgent,
    input: "Summarize today's activity.",
    scheduleExpression: "cron(0 9 * * ? *)",
    timezone: "Europe/Amsterdam",
  },
});
```

Or use `events` instead of `input` for multimodal payloads:

```ts
config: {
  agent: myAgent,
  events: [
    { role: "user", content: [{ type: "text", text: "Check status." }] },
  ],
  scheduleExpression: "rate(1 hour)",
}
```

## Channels

Each channel has a dedicated constructor:

```ts
import {
  defineTelegramChannel,
  defineDiscordChannel,
  defineSlackChannel,
  defineGitHubChannel,
  definePancakeChannel,
  defineZaloChannel,
} from "filthy-panty";
```

A channel definition must be attached to **exactly one** agent. An agent may have multiple channels, but not multiple of the same type.

## Project Config

Optionally export a `defineFilthyPanty` config to set project defaults:

```ts
import { defineFilthyPanty } from "filthy-panty";

export default defineFilthyPanty({
  project: "my-project",
  environments: {
    dev: "development",
    deploy: "production",
  },
  dashboardUrl: "https://dashboard.beeblast.co",
});
```

These values can be overridden by CLI flags (`--project`, `--env`) or `.env.local`.

## Validation

The CLI validates resource configs at compile time:

- Unknown agent config keys are rejected with suggestions (e.g. `workspace:` → did you mean `workspaces:`?).
- Duplicate resources are rejected.
- Skill bundles must contain `SKILL.md`.
- Tool bundles must build successfully as ESM.
- Workspace storage provider must be `s3`.
- Sandbox mounts must support S3 workspace access.

These checks run during `filthy-panty dev` and `filthy-panty deploy`, so you get fast feedback without deploying broken config.

## Generated References

After `filthy-panty dev` or `filthy-panty deploy`, the CLI writes `_generated/api.ts` with typed references:

```ts
import { api } from "./filthypanty/_generated/api";

// api.agents.myAgent      → AgentReference
// api.channels.myTelegram → ChannelReference
// api.workspaces.notes    → string (workspaceId)
// api.sandboxes.lambda    → string (sandboxId)
```

Use these references with `FilthyPantyClient` for fully typed runtime calls:

```ts
import { FilthyPantyClient } from "filthy-panty";
import { api } from "./filthypanty/_generated/api";

const client = new FilthyPantyClient();
await client.run(api.agents.myAgent, { input: "Hello!" });
```

## Raw API Fallback

The code-first layer compiles to the same manifest the raw account API accepts. If you ever need to call the REST API directly, the shapes are identical. See the [API Reference](/api-reference) for the full OpenAPI spec.
