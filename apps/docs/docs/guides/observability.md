...
| Tracing    | Every run as a trace: the full system prompt, tools offered, model input, reasoning, response, each tool call with input and output |
...
Runs are labelled by what started them: `task` for a request, `cron` for a scheduled run, `subtask` for a subagent. Each model step is split into time to first token, streaming, and tool wait, so a slow tool never looks like slow generation. A failed `task` or `cron` run has a Continue button. See [Conversations](conversations.md).

The system prompt shown on a trace is the whole assembled prompt: your `agent.system` plus memory, workspace, skills, subagent and scheduler blocks, and any steering. Large payloads are truncated, but the recorded size is always real.
...
- Secrets are redacted before anything is written: by field name, and by matching the run's known secret values.
...
For your own pipeline, send [webhooks](webhooks.md) to a service you run. How the pipeline works inside: [Observability internals](../internals/observability.md).
