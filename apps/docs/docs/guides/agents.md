...
An agent is a model plus everything it may use: a system prompt, tools, sandboxes, workspaces, channels, skills, subagents and hooks. This page covers the model and prompt side. Each capability has its own guide, linked at the end.
...
Set one level in `model.reasoning` and it works across providers: `provider-default`, `none`, `minimal`, `low`, `medium`, `high` or `xhigh`.
...
`agent.system` is the base prompt. Broods appends blocks to it when the matching feature is on: workspace guidance, the memory index, available skills, subagents, and the scheduler clock. The Tracing tab in the dashboard shows the full assembled prompt for every run.
...
| `allowRunOverrides: true` | off     | A runtime-key caller may send `system` messages and `model` overrides. Without it: `403 run_overrides_disabled`. |
...
By default an agent runs on the Broods loop: a serverless AI SDK `streamText` loop with the Broods tools. `defineHarness` swaps that loop for another agent runtime that runs inside the agent's first sandbox: Claude Code, Codex, Deep Agents, OpenCode or Pi.
...
What changes with a harness:

- The first sandbox must be persistent and use the `sandbox` or `lambda` provider.
- The harness ships its own file tools, so the Broods workspace prompt is off. Memory (`memory_save`) still works.
...
| `deepagents`  | `anthropic`, `vercel`                   | yes           | auto-rejects   |
| `opencode`    | `anthropic`, `openai`, `vercel`         | yes           | auto-rejects   |
| `pi`          | its own catalog, keyed by your provider | yes           | yes            |
...
- [Tools](tools.md): provider tools and MCP servers.
- [Sandboxes](sandboxes/index.md) and [Workspaces](workspaces.md): run code and keep files.
- [Channels](../channels/index.md): answer in chat apps.
- [Skills](skills.md), [Subagents](subagents.md), [Scheduling](scheduling.md).
- [Hooks](hooks.md), [Webhooks](webhooks.md), [Policies](policies.md): observe and control runs.
