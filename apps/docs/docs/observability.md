...
A `subscribe` that asks for `backfill` always gets a closing `backfill` message back,
even when Loki or Tempo could not answer: that message then carries `error`, so a
