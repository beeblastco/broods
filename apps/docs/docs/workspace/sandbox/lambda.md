...
- `microvmArtifactsBucketName` is the S3 bucket for the zipped image artifacts (`microvm-images/`).
- `microvmBuildRoleArn` is the build role for `CreateMicrovmImage` / `UpdateMicrovmImage`.
- `microvmExecutionRoleArn` is the runtime role for `RunMicrovm` + CloudWatch logs.
...
- On idle the VM is **suspended** (a Firecracker snapshot of memory + disk); the next call
  **resumes** it, and restore is ~1-10 s. The idle/expiry policy comes from `config.lifecycle`
...
Code and Codex Harness bridges inside a persistent MicroVM. That path is
validated and working, but the main Broods agent run loop does not select it
yet.
...
Core mints the AWS token just in time, scoped to the declared bridge port. It
...
`restricted` currently behaves exactly like `deny-all`. A shared, deploy-time network
boundary enforces egress, so it cannot take a per-account `allowDomains` / `allowCidrs`
allowlist; a `lambda` config that sets either is rejected at validation rather than
accepted and ignored. Prefer `deny-all`, which states what you get.
