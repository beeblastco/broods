...
| `lambda`  | default `INTERNET_EGRESS`           | VPC egress connector (no internet) | same as `deny-all`; `allowDomains` / `allowCidrs` are rejected at validation; fails closed if no connector is provisioned |
