...
- **Session** (`fp_sts_...`): minted by `POST /v1/account/assume-role` with `{ roleId, ttlSeconds? }`. Default TTL is 1 hour, maximum 12. Only the hash is stored; the token is shown once. A session authenticates the same `/v1/*` routes as the account secret but every request must match the role's policy. No matching allow rule means 403.

Three credentials may call assume-role:

1. The account secret (`fp_acct_...`), the human path. Hold the master credential, mint a narrow session for whatever tool you are about to hand it to.
2. A CLI login token (`fp_cli_...`), the same path for `broods` CLI users.
