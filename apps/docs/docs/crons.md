...
- **Anything else**, including the `cron:<cronId>` default, gets its own direct conversation. The result is readable through the async status API; nothing is pushed anywhere.
...
A fired run carries no scheduling tools at all, however the agent is configured: not `schedule`, not `update_schedule`, not `list_schedules`, not `cancel_schedule`. A model reading its own stored instructions takes them for a fresh request, so every one of those tools is a way for it to act on the schedule it is currently running. A subagent dispatched by a fired run inherits the same restriction. Scheduling stays with the turns a person actually asked for.
...
A one-time `at(...)` job deletes itself: its scheduled run is spent as soon as it has fired, and the runtime deletes the cron job and its run history once that single run settles. `GET /v1/crons/{cronId}` returns 404 from then on, so read the result through the conversation or the async status API rather than the job. Recurring jobs are never deleted on their own.
...
An agent can manage its own scheduled work with three tools. They are off by default, because a scheduled task starts billable runs long after the turn that asked for it. They turn on together, per agent:
...
| `list_schedules`  | none                                                                    | Every job owned by this agent, with its schedule, status, and conversation |
...
These are normal cron jobs through the same config plane. They show on the dashboard scheduler page, where creating, editing or deleting one takes the org admin role, and they are manageable through `/v1/crons` like any other. Two things `schedule` fixes for the model:
...
Besides the declarative `defineCron` resources that `broods dev` syncs, clients can create, update, and delete cron jobs at runtime through the SDK. It calls the same account API, so the registered schedules stay in sync:
