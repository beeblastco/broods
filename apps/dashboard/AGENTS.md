...
- component file name is CamelCase, **except `app/components/ui/`**. that folder is vendored shadcn/ui: keep the upstream name (`button.tsx`, `dropdown-menu.tsx`). rename it and the next `shadcn add` writes the lowercase file back, which is invisible on case-insensitive macOS and two colliding files on linux CI.
