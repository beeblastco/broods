...
- `bun run test:app` and `bun run perf` are the signed-in suites, against a real Convex and WorkOS. Both need `E2E_EMAIL` + `E2E_PASSWORD`, a WorkOS user with email + password auth; `e2e/auth.setup.ts` signs it in once, provisions its org and default project on a fresh account, and saves the session under `e2e/.auth/`. `E2E_BASE_URL` picks the server: a deployment, or leave it unset for `http://localhost:3000`, where the config starts `next dev` on your `.env.local` (self-hosted Convex) unless one is already running. Optional `E2E_PROJECT_ID` pins the project.
  ...
  - The probe login for local runs (`e2e-probe@beeblast.co`, WorkOS Staging) is already saved as `E2E_EMAIL` / `E2E_PASSWORD` in the main checkout's `apps/dashboard/.env.local`. A worktree has no `.env.local`: copy it from the main checkout, then `bun --env-file=.env.local x playwright test --project=setup` signs in and every signed-in suite can run. Never reset the probe's password without also running `gh secret set DASHBOARD_E2E_PASSWORD --env development`: CI signs in as the same user, and every PR's `app-surfaces` fails until the secret matches.
  - For real usage data on dev, the probe's project (`silver-echo`, Development) has a `usage-probe` agent on `deepseek-flash` with public access. Read the stage key from the dashboard's API key tab ("Reveal key") and send prompts with `BroodsClient.run({ agentId, conversationKey, input })` against `https://gateway.dev.broods.app`; the Agent ID is on the agent's Details tab.
