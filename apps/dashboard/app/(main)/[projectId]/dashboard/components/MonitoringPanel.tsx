...
function formatDateTime(ms: number): { date: string; time: string } {
    const d = new Date(ms);
    const date = d.toLocaleDateString([], { month: "short", day: "2-digit" }).toUpperCase();
 ...
 */
function parseLogMessage(raw: string): { summary: string; pretty: string; eventType?: string } {
...
function shortFunctionName(name: string): string {
    return name.replace(/-ap-[a-z]+-\d+-\d{6,}$/i, "").replace(/^filthy-panty-/, "");
}
    ...
    return (
        <>
            ...
            >
                ...
                <td className="px-3 py-1.5 whitespace-nowrap text-muted-foreground/80 max-w-[200px] truncate" title={entry.functionName}>
                ...
                <tr className="border-b border-border/40 bg-background/40">
                    <td colSpan={4} className="px-3 py-3">
                        <div className="flex flex-col gap-2">
                            <div className="text-[10px] text-muted-foreground/70 break-all" title={entry.functionName}>
                                {entry.functionName}
                            </div>
    ...
    return (
        <div className="grid gap-8">
            ...
            >
                ...
                {fetchError && <p className="mb-3 text-sm text-destructive">{fetchError}</p>}
