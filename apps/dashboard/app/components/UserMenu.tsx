...
export function UserMenu() {
    ...
    if (isLoading) {
        return (
            <button aria-label="Loading account" className="relative flex size-6 items-center justify-center rounded-full ring-1 ring-white/10" disabled type="button">
                <Avatar size="sm"><AvatarFallback className="bg-muted text-[10px] font-medium text-muted-foreground">...</AvatarFallback></Avatar>
            </button>
        );
    }
            ...
            <DropdownMenuTrigger asChild>
                <button className="relative flex size-6 items-center justify-center rounded-full ring-1 ring-white/10 transition-all hover:ring-white/25 focus:outline-none data-[state=open]:ring-2 data-[state=open]:ring-white/40">
                    <Avatar size="sm">
                        ...
                        <AvatarFallback className="bg-muted text-[10px] font-medium text-muted-foreground">{initials}</AvatarFallback>
                    </Avatar>
                </button>
            </DropdownMenuTrigger>
