...
export function OrgSwitcher() {
    ...
    const label = active?.name ?? (active === null ? "No organization" : "Loading...");

    return (
        <>
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <button className="flex items-center gap-1.5 rounded-md px-2 py-1 text-sm font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground focus:outline-none data-[state=open]:bg-accent data-[state=open]:text-foreground cursor-pointer">
                    ...
                    {orgs === undefined ? (
                        ...
                        orgs.map((org) => (
                            ...
                            >
                                ...
                                {active?._id === org._id && <Check className="size-3.5 text-muted-foreground" />}
                            </DropdownMenuItem>
                        ))
                    )}
                    <DropdownMenuSeparator />
            ...
            <Dialog open={createOpen} onOpenChange={setCreateOpen}>
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="grid gap-2 py-2">
                        <Label htmlFor="new-org-name" className="text-xs text-muted-foreground">Name</Label>
                        <Input
                            id="new-org-name"
                            value={newName}
