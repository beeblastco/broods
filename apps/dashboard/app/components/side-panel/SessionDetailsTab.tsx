    ...
    <div className="flex flex-1 flex-col gap-5 overflow-y-auto p-4">
      ...
      {disabled ? (
        <p className="text-xs text-muted-foreground">
          Wire this session store to an agent to tune retention.
        </p>
      ) : (
        <div className="flex flex-col gap-3">
          ...
          <ToggleRow
            ...
            onCheckedChange={(next) =>
              updateBranch(["session", "pruning", "enabled"], next)
            }
          />
          <ToggleRow
            label="Compaction"
            ...
            disabled={disabled}
            onCheckedChange={(next) =>
              updateBranch(["session", "compaction", "enabled"], next)
            }
          />
          {compactionEnabled && (
            <ExpandBlock>
              <div className="flex flex-col gap-1.5">
                ...
                <Input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter")
                      setMaxContextLength((e.target as HTMLInputElement).value);
                  }}
                />
              </div>
            </ExpandBlock>
