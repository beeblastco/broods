...
export function WorkspaceDetailsTab(props: {
  ...
  const workspace = useMemo(
    () =>
      readAgentBranch<WorkspaceSlice>(
  ...
  const committedMultiple =
    !!workspace.workspaces && Object.keys(workspace.workspaces).length > 0;
  const [multipleWorkspaces, setMultipleWorkspaces] =
    useState(committedMultiple);
  const [prevCommittedMultiple, setPrevCommittedMultiple] =
    useState(committedMultiple);
      ...
      <div className="flex flex-col gap-3">
        ...
        <ToggleRow
          ...
          onCheckedChange={(next) =>
            updateBranch(["workspace", "enabled"], next)
          }
        />
        <ToggleRow
          ...
          onCheckedChange={(next) =>
            updateBranch(["workspace", "needsApproval"], next)
          }
        />
        <ToggleRow
          label="Guidance"
          ...
          disabled={disabled || !workspaceEnabled}
          onCheckedChange={(next) =>
            updateBranch(["workspace", "harness", "workspace", "enabled"], next)
          }
        />
        <ToggleRow
          label="Memory"
          ...
          disabled={disabled || !workspaceEnabled}
          onCheckedChange={(next) =>
            updateBranch(["workspace", "harness", "memory", "enabled"], next)
          }
        />
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        {multipleWorkspaces ? (
          ...
          <AutoText
            ...
            disabled={disabled || !workspaceEnabled}
            onCommit={(v) =>
              updateBranch(["workspace", "namespace"], v || undefined)
            }
          />
        )}
      </div>
        ...
        <div className="flex flex-col gap-3">
          <ToggleRow
            ...
            onCheckedChange={(next) =>
              updateBranch(
                ["workspace", "sandbox"],
                next ? { provider: "lambda" } : undefined,
              )
            }
          />
          {workspace.sandbox && (
...
const SANDBOX_PROVIDERS: {
  value: NonNullable<SandboxSlice["provider"]>;
  label: string;
}[] = [
...
function SandboxConfig({
  ...
  const envVars = isPlainObject(sandbox.envVars)
    ? (sandbox.envVars as Record<string, string>)
    : {};
  const setOption = (key: string, value: unknown) =>
    updateBranch(["workspace", "sandbox", "options", key], value);
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <AutoNumber
          ...
          disabled={disabled}
          onCommit={(v) =>
            updateBranch(["workspace", "sandbox", "memoryLimit"], v)
          }
        />
        <AutoNumber
          label="Output bytes"
          ...
          disabled={disabled}
          onCommit={(v) =>
            updateBranch(["workspace", "sandbox", "outputLimitBytes"], v)
          }
        />
      </div>
    ...
    return (
      <OptionsBlock label="Daytona options">
        ...
        <ToggleRow
          ...
          disabled={disabled}
          onCheckedChange={(v) =>
            setOption("mountAwsS3Buckets", v || undefined)
          }
        />
        <ToggleRow
  ...
  return (
    <div className="flex flex-col gap-2">
      <p className="text-[11px] text-muted-foreground">
        Map workspace ids to namespaces. Mark one as the default the agent opens
        with.
      </p>
      {rows.map((row, index) => {
        ...
        return (
          <div
            key={index}
            className="flex flex-col gap-1.5 rounded-lg border border-border/70 bg-muted/20 p-2.5"
          >
            <div className="flex items-center gap-1.5">
              ...
              <Button
                ...
                onClick={() =>
                  updateBranch(["workspace", "defaultWorkspace"], id)
                }
              >
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {rows.map(([k, v], index) => (
        <div
          key={index}
          className="grid grid-cols-[1fr_1fr_auto_auto_auto] items-center gap-1.5"
        >
          <Input
            value={k}
            placeholder="KEY"
