...
import { analyzeCanvasInfra, deriveSubagentRefs } from "../app/lib/canvasRuntimeRefs";
...
function node(id: string, type: string, data: Record<string, unknown> = {}): Node {
    return { id: id, type: type, position: { x: 0, y: 0 }, data: { label: id, ...data } } as Node;
}

function edge(source: string, target: string, type?: string): Edge {
    return { id: `${source}->${target}`, source: source, target: target, ...(type ? { type: type } : {}) } as Edge;
}
