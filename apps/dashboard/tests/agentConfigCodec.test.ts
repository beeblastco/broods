...
import { applyModelReasoning, fromNestedAgentConfig, readModelReasoning, toNestedAgentConfig } from "../app/lib/agentConfigCodec";
...
describe("agent config codec", () => {
    ...
    test("rejects unsupported config keys instead of dropping them", () => {
        expect(() => toNestedAgentConfig({
            extraConfig: { model: { options: { anthropic: { thinking: { type: "enabled" } } } } },
        ...
        expect(() => toNestedAgentConfig({
            extraConfig: { workspace: { sandbox: { filesystem: { enabled: true } } } },
...
describe("model reasoning codec", () => {
    test("maps budget tokens to the Anthropic thinking provider option", () => {
        ...
        expect(model).toEqual({ providerOptions: { anthropic: { thinking: { type: "enabled", budgetTokens: 4096 } } } });
    ...
    test("routes MiniMax budget through the Anthropic-compatible slot", () => {
        ...
        expect(model).toEqual({ providerOptions: { anthropic: { thinking: { type: "enabled", budgetTokens: 2048 } } } });
    });

    test("maps OpenAI effort to reasoningEffort", () => {
        const model = applyModelReasoning({}, "openai", { effort: "high" });

        expect(model).toEqual({ providerOptions: { openai: { reasoningEffort: "high" } } });
    ...
    test("maps Google budget to thinkingConfig.thinkingBudget", () => {
        ...
        expect(model).toEqual({ providerOptions: { google: { thinkingConfig: { thinkingBudget: 8192, includeThoughts: true } } } });
    ...
    test("deep-merges reasoning providerOptions with sibling options from the flat column", () => {
        ...
        const nested = toNestedAgentConfig({
            providerOptions: { anthropic: { cacheControl: { type: "ephemeral" } } },
            extraConfig: {
                model: { providerOptions: { anthropic: { thinking: { type: "enabled", budgetTokens: 4096 } } } },
            },
        });
