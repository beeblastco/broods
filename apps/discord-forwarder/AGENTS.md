...
- **the resume host is a credential decision.** the RESUME frame carries the bot token, and READY names its own host in `resume_gateway_url`. `isDiscordGatewayUrl` in `src/discord.ts` accepts only a `wss:` URL on a Discord host and falls back to the default gateway; following a redirect elsewhere would hand the token to whoever named it. `heartbeat_interval` from HELLO is clamped for the same reason — an unbounded value from the wire becomes a hot loop.
