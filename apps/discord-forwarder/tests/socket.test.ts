...
describe("gateway socket", () => {
  ...
  it("re-dials after an unacknowledged heartbeat without a close event", () => {
  ...
  it("ignores the late close event of a socket it already abandoned", () => {
    const { gateway, first } = readySocket();
    jest.advanceTimersByTime(UNTIL_REDIAL_MS);
  ...
  it("identifies afresh after a non-resumable INVALID_SESSION", () => {
    const { gateway, first } = readySocket();
