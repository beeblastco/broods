...
export async function enforceAccountSignupRateLimit(request: CoreRequest): Promise<void> {
  ...
  try {
    await dynamo.send(new UpdateItemCommand({
      ...
      Key: {
        rateLimitKey: { S: `account-signup:${sourceHash(request)}:${windowStart}` },
      },
      UpdateExpression: [
...
function sourceHash(request: CoreRequest): string {
  const sourceIp = request.clientIp || "unknown";
