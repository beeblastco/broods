...
export interface AgentConfig {
  ...
  // Lets a runtime-key caller send `system` messages and `model` overrides.
  // Off by default: the key is embeddable, so whoever holds it would otherwise
  // pick the prompt and the spend. Stage tickets and account secrets ignore it.
