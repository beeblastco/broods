...
- Two handlers, one process, split by path in `src/server.ts` (`routesToAccountManage`):
  - `src/accounts/handler.ts` — account creation, secret rotation, account metadata/config management (`/v1/account`, `/v1/{agents,crons,policies,sandboxes,workspaces}`, admin `/accounts/*`), plus the `/v1/internal/observability-log` service-token leaf (the Convex config-audit bridge). Skills, tools, and workspace-files CRUD are NOT core routes: they live in the Convex config plane (`packages/convex/configHttp.ts`) and the gateway forwards `/v1/skills*`, `/v1/tools*`, and `/v1/workspaces/{id}/files` there (`BROODS_CONFIG_URL`). Core keeps only their runtime read paths (`src/shared/skills.ts`, tool bundle loading) and account-deletion cleanup.
