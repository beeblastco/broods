...
`@broods/core` is the agent harness. one Bun container (Vercel AI SDK) serving the whole runtime behind the gateway.
...
- **most CRUD is not here.** account metadata/rotate, and agent, skills, mcp, workspace-files, cron, workspace, sandbox-config, policy, channel-record CRUD all live in the Convex config plane (`../../packages/convex/config/http.ts`); gateway send those paths there by `BROODS_CONFIG_URL`. core keep only the runtime side: `POST /v1/agents/{id}` and scoped invocation, skills, mcp bundle load, workspace mount/S3 read, sandbox lifecycle verbs, the `/v1/cron-runs` leaf, account-delete cleanup. hunting for account CRUD in core is wasted grep.
...
- hosted MCP servers: `src/harness/mcp/hosted.ts` invokes the Lambda whose source is `../lambda/`. plain `.mjs`, no build, `sst.config.ts` points straight at it; `src/harness/frames.ts` is the NDJSON frame protocol it shares with the isolate tier and `hook-runner.ts`. the MicroVM sandbox that runs bash/python is a different plane; keep MCP host code out of `src/harness/sandbox/`. the Lambda answers over `InvokeWithResponseStream`, not `Invoke`. its response is raw NDJSON, so core and the Lambda must roll together. the Lambda **handler** resolves `bundleUrl`, not the child: the handler process is warm across invocations and keeps its connection to S3. one invoke carries a **batch** (#397): calls to the same `accountId:sha256` inside `MCP_BATCH_WINDOW_MS` ride one `requests[]` payload, `MCP_BATCH_MAX` per invoke. child frames are tagged with the request `id` and the batch closes on `end`; an `error` **without** an id kills the batch and retires the child. `t` first, `id` second on every child frame is load-bearing: the handler spots the terminal by byte prefix. a tenant bundle must hand `createMcpHandler` a factory that builds a **fresh** server per request: a batch runs its requests concurrently in one process, and the SDK aborts every in-flight handler on a shared instance when one request's transport closes.
- **hosted MCP servers stay on Lambda, never move to a Worker.** they are meant to stop being TypeScript-only. python, go, rust uploads need a real process, a filesystem and real deps, which a V8-isolate Worker cannot give. a Worker is right only for code _we_ vendor and ship (#191); tenant code is not that.
- the isolate tier is not a Node runtime and not a browser. what a hook bundle can reach is exactly `isolate/runner/web-globals.mjs` plus the timers/fetch/console set in `runner.mjs`. `isIsolateSafeBundle` (single home: `packages/convex/model/isolateSafety.ts`) is what rejects a bundle the isolate cannot run at upload time, so widening one without the other turns an upload check into a runtime ReferenceError. no Web Streams there: anything importing `ai` is refused.
...
- **a tenant bundle can never be imported from a `data:` URL.** bundlers emit `createRequire(import.meta.url)` at module scope whenever they inline a CommonJS dep, and `createRequire` rejects a data: URL outright. the server dies before its own first line. `child-runner.mjs` serves the bytes under a synthetic `file:` URL through `module.registerHooks`; nothing is written to disk.
- a hosted MCP bundle default-exports a **fetch-style handler** (`createMcpHandler(...)`); each request in a batch is exactly one serialized web request. **secrets ride request headers** as `${NAME}` refs Convex resolved into the agent config at sync time. there is no `process.env` in the child (it is scrubbed).
- the mcp child stamps **`cpuUsec` on its terminal frame**, and core meters it as `type: "mcp-sandbox"`, `role: "tool"`, never as the `lambda` sandbox provider, which means the agent's own MicroVM and would file hosted-MCP compute under the agent's sandbox. that sample is what feeds `usageRollups.toolSandboxCpuUsec` and the dashboard Compute panel. `toolCallId` rides the in-memory sample only, for the span; the stored row stays aggregated.
- **one normalized duration per tool call.** `toolSpanDurationMs` picks the SDK's `toolExecutionMs` over the handler clock (which overstates parallel calls by the model's own time) and clamps NaN/negative. span, observability row, `tool.call.finished`, summary and logs all quote that one number, so a trace and its event log cannot disagree.
...
- cron come from outside: the Convex crons component (`packages/convex/agent/crons.ts` `dispatch`) POST → gateway `/v1/cron-runs` (service-token auth) → `handleScheduledCron`. a cron `conversationKey` that name a **live channel session** make the run resume that session and reply there (`getConversationTarget` → `replyTarget`); anything else stay its own `api:` conversation. `schedule` / `list_schedules` / `update_schedule` / `cancel_schedule` are the runtime way in. the only cron writes core own, straight to `agent/crons` through `storage.crons`. a fired run is marked by `session.trigger === "cron"`: that one flag name the root span `agent.cron` **and** withhold **every** schedule tool, because the run read its own stored instructions as a fresh request and would act on the job it is running. subagent inherit the flag (their span stay `subtask`). those instructions never reach the model bare either. `withScheduledRunContext` frame the first user message with what fired and when, since nobody typed that turn. an `at(...)` job is **self-deleting**: its scheduled run is spent once fired, and core drop the row when that one run settle (`settleCronRun`) or when it never start. so cron row deletion is a normal path now, not only an owner action. never assume the row still there after a run.
