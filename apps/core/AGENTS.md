...
- hosted MCP servers: `src/harness/mcp/hosted.ts` invokes the Lambda whose source is `../lambda/` — plain `.mjs`, no build, `sst.config.ts` points straight at it; `src/harness/frames.ts` is the NDJSON frame protocol it shares with the isolate tier and `hook-runner.ts`. the MicroVM sandbox that runs bash/python is a different plane; keep MCP host code out of `src/harness/sandbox/`. the Lambda answers over `InvokeWithResponseStream`, not `Invoke` — its response is raw NDJSON, so core and the Lambda must roll together. the Lambda **handler** resolves `bundleUrl`, not the child: the handler process is warm across invocations and keeps its connection to S3. one invoke carries a **batch** (#397): `hosted.ts` holds a call `MCP_BATCH_WINDOW_MS` (default 10, 0 = off) so the sibling calls of one model step to the same `accountId:sha256` ride one `requests[]` payload, `MCP_BATCH_MAX` (default 8) per invoke. the child answers each request on a frame tagged with its `id` and closes the batch on `end`; an `error` **without** an id is the whole batch dead and retires the child, an error **with** one fails that request only. the handler spots the terminal by byte prefix, so `t` first and `id` second on every child frame is load-bearing. every call in a batch resolves together, after `end`, because the harness reads a call's `cpuUsec` share the moment its result lands.
...
- a hosted MCP bundle default-exports a **fetch-style handler** (`createMcpHandler(...)`); one invoke carries exactly one serialized web request. **secrets ride request headers** as `${NAME}` refs Convex resolved into the agent config at sync time — there is no `process.env` in the child (it is scrubbed).
