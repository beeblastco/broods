...
- account-uploaded tools live in `src/harness/custom-tools/` — `executor.ts` dispatches by runtime tier, `payload.ts` is the frame protocol both tiers share. pure/fetch-only bundles run in the in-core V8 isolate (`src/harness/isolate/`); node/npm/native bundles run in the tool-runner Lambda, whose source is `tool-runner/` at the app root because it deploys separately from the container. the MicroVM sandbox that runs bash/python is a separate plane — do not put custom-tool code in `src/harness/sandbox/`.
- `.mjs` under `tool-runner/` and `isolate/runner/` is deliberate: those files are spawned as raw `node` processes, so they ship as runnable ESM rather than through the Bun/TS build.
