...
- `../../packages/convex` (`@broods/convex`): shared Convex backend. Core's adapter modules in `src/shared/convex/` read it, `src/shared/core-store.ts` exposes the runtime-facing store boundary, and `src/shared/domain/` owns shared records/config codecs. Every stage requires both `CONVEX_URL` and `CONVEX_DEPLOY_KEY`. Read `../../packages/convex/AGENTS.md` before changing Convex files.
