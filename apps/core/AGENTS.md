...
- account-uploaded tools live in `src/harness/custom-tools/` — `executor.ts` dispatches by runtime tier, `payload.ts` is the frame protocol both tiers share. pure/fetch-only bundles run in the in-core V8 isolate (`src/harness/isolate/`); node/npm/native bundles run in the tool-runner Lambda, which is its **own workspace** at `../tool-runner` (`@broods/tool-runner`) — it deploys as a separate function, not into the container binary. `sst.config.ts` points at its built `dist/`, so `bun run build` must run before a deploy. the MicroVM sandbox that runs bash/python is a separate plane — do not put custom-tool code in `src/harness/sandbox/`.
- `isolate/runner/*.mjs` is deliberate: those files are spawned as raw `node` processes (Bun cannot load isolated-vm), so they ship as runnable ESM rather than through the TS build.
