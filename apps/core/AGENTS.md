...
- account-uploaded tools run by runtime tier: pure/fetch-only bundles use the in-core V8 isolate; node/npm/native bundles use the synchronous tool-runner Lambda. the MicroVM sandbox that runs bash/python is a separate plane.
