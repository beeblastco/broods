...
describe("code hooks end-to-end (real isolate)", () => {
  realRunnerIt(
    "injects a system prompt from an agent.started hook",
    async () => {
  ...
  realRunnerIt("denies a real tool call through the tool wrapper", async () => {
    process.env.TOOL_BUNDLES_BUCKET_NAME = "test-bundles";
    const { createHookDispatcher, wrapToolsWithHooks } =
      await import("../src/harness/hook-dispatcher.ts");
    const dispatcher = createHookDispatcher(
      "acct_test",
      indexFor(["agent.started", "tool.call.started"]),
    ...
    const allowed = await (
      wrapped.read!.execute as (i: unknown, o: unknown) => Promise<unknown>
    )({ path: "a.txt" }, {});
  ...
  realRunnerIt("threads ctx.state across hooks in one run", async () => {
    ...
    await dispatcher.runMutation("agent.started", {
      system: "You are helpful.",
      messages: [],
    });
    const finished = await dispatcher.runMutation("agent.finished", {
      finishReason: "stop",
      response: "hi",
    });
  ...
  realRunnerIt(
    "serializes overlapping hook runs so concurrent ctx.state writes are not lost",
    async () => {
      process.env.TOOL_BUNDLES_BUCKET_NAME = "test-bundles";
      const { createHookDispatcher } =
        await import("../src/harness/hook-dispatcher.ts");
  ...
  realRunnerIt(
    "shapes subagent visibility and drops/rewrites channel messages",
    async () => {
      process.env.TOOL_BUNDLES_BUCKET_NAME = "test-bundles";
      const { createHookDispatcher } =
        await import("../src/harness/hook-dispatcher.ts");
      ...
      expect(
        await dispatcher.runMutation("subagent.task.finished", {
          taskId: "t1",
          result: "long output",
        }),
      ).toEqual({ visibleResult: "summary of t1" });
      expect(
        await dispatcher.runMutation("channel.message.received", {
          channel: "telegram",
          text: "spam",
        }),
      ).toEqual({ drop: true });
...
describe("channel.message.received rewrite reaches the session", () => {
  ...
  realRunnerIt(
    "forwards the rewritten ingress events to the channel handler",
    async () => {
      process.env.TOOL_BUNDLES_BUCKET_NAME = "test-bundles";
      const { createIncomingEventRouter } =
      ...
      const hookRecord = indexFor(["channel.message.received"]).get(
