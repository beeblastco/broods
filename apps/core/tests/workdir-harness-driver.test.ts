...
describe("WorkdirHarnessDriver", () => {
  test("maps an existing core reservation to command, file, port, and lifecycle operations", async () => {
    ...
    expect(fake.launchCommands[0]).toContain("&\n__attempt=0");
    ...
    expect(portUrl.origin + portUrl.pathname).toBe(
      "wss://workdir.example.test/ports/4321",
    );
    expect(portUrl.searchParams.get("key")).toBe("test-key");
  ...
  test("accepts the recipe identity supplied dynamically by HarnessAgent", async () => {
    const fake = fakeWorkdir();
    const executor = fakeExecutor(fake.sandbox, true);
