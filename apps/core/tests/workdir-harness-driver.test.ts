...
describe("WorkdirHarnessDriver", () => {
  test("maps an existing core reservation to command, file, port, and lifecycle operations", async () => {
    ...
    expect(executor.acquisitions).toEqual([{ reservationKey: "acct:agent:harness", abortSignal: signal }]);
    ...
    await created.session.writeFile({ path: "/workspace/data.bin", content: new Uint8Array([0, 1, 255]) });
    expect(await created.session.readFile({ path: "/workspace/data.bin" })).toEqual(new Uint8Array([0, 1, 255]));
    expect(await created.session.readFile({ path: "/workspace/missing" })).toBeNull();
    expect(await created.session.getPortUrl?.({ port: 4_321, protocol: "ws" })).toBe(
      "wss://workdir.example.test/ports/4321",
    );

    await created.session.stop();
    await created.session.destroy?.();
    expect(executor.suspensions).toEqual([{ reservationKey: "acct:agent:harness" }]);
    expect(executor.releases).toEqual([{ reservationKey: "acct:agent:harness" }]);
  });
  ...
  test("resumes the same reservation and rejects a mismatched bootstrap identity", async () => {
    ...
    await expect(driver.createSession({ identity: "other-bootstrap" })).rejects.toThrow(
      "bootstrap identity does not match",
    );

    const resumed = await driver.resumeSession?.({ sessionId: "session-1" });
    expect(resumed?.id).toBe("workdir-1");
    expect(executor.resumptions).toEqual([{ reservationKey: "acct:agent:harness" }]);
  ...
  test("kills a running Workdir process and rejects with the abort reason", async () => {
    ...
    const command = session.runCommand({ command: "sleep 60", abortSignal: controller.signal });
  ...
  test("releases a newly claimed reservation when creation is aborted after allocation", async () => {
    const fake = fakeWorkdir();
    const controller = new AbortController();
    const failure = new DOMException("cancelled after allocation", "AbortError");
    const executor = fakeExecutor(fake.sandbox, true, () => controller.abort(failure));
    const driver = new WorkdirHarnessDriver(driverOptions(), executor.value as never);

    await expect(
      driver.createSession({ identity: "bootstrap-v1", abortSignal: controller.signal }),
...
function driverOptions(): WorkdirHarnessDriverOptions {
  return {
    ...
    config: {
      provider: "sandbox",
      persistent: true,
      options: { workdirUrl: "https://workdir.example.test", apiKey: "test-key" },
    },
...
function fakeExecutor(sandbox: Sandbox, isFirstCreate: boolean, afterAcquire?: () => void) {
  ...
  const sandbox = {
    ...
    async exec(command: string) {
      ...
      if (command.includes("echo \"done $(cat") && processRoot) {
        ...
        return result(process.running ? "running\n" : `done ${process.exitCode}\n`);
      }

      const readPath = command.match(/base64 < '([^']+)'/)?.[1];
      if (readPath) {
        const content = files.get(readPath);
        return content === undefined ? result("", 44) : result(Buffer.from(content).toString("base64"));
      }
