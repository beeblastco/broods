...
describe("sanitizeHookResult", () => {
  it("keeps only the mutable fields for the event", () => {
    ...
    expect(
      sanitizeHookResult("tool.call.started", {
        decision: "deny",
        denyReason: "no",
        extra: true,
      }),
    ...
    expect(
      sanitizeHookResult("channel.message.sending", {
        drop: true,
        text: "edited",
        secret: "x",
      }),
  ...
  it("returns undefined for observe-only events", () => {
    expect(
      sanitizeHookResult("agent.step.finished", { anything: 1 }),
    ).toBeUndefined();
    expect(
      sanitizeHookResult("tool.call.finished", { output: 1 }),
    ).toBeUndefined();
  ...
  it("returns undefined when the return is not a plain object or has no mutable field", () => {
    ...
    expect(
      sanitizeHookResult("agent.started", { unrelated: 1 }),
    ).toBeUndefined();
  });
...
describe("isolate runner hook mode", () => {
  ...
  realRunnerIt(
    "invokes the handler for the fired event and returns its value",
    async () => {
      ...
      expect(result.frames).toEqual([
        {
          t: "final",
          result: { result: { system: "Base. Be terse." }, state: {} },
        },
      ]);
    },
  );
  ...
  realRunnerIt("routes each event to its own handler", async () => {
    ...
    const denied = await runHookRunner(source, "tool.call.started", {
      toolCall: { toolName: "bash" },
    });
    expect(denied.frames).toEqual([
      { t: "final", result: { result: { decision: "deny" }, state: {} } },
    ]);
    const allowed = await runHookRunner(source, "tool.call.started", {
      toolCall: { toolName: "read" },
    });
    expect(allowed.frames).toEqual([
      { t: "final", result: { result: { decision: "allow" }, state: {} } },
    ]);
  });

  realRunnerIt(
    "exposes ctx.state and returns it after the hook mutates it",
    async () => {
      const result = await runHookRunner(
      ...
      expect(result.frames).toEqual([
        { t: "final", result: { result: { system: "s" }, state: { seen: 5 } } },
      ]);
    },
  );

  realRunnerIt(
    "returns no mutation and unchanged state when the bundle has no handler for the event",
    async () => {
      const result = await runHookRunner(
      ...
      expect(result.frames).toEqual([
        { t: "final", result: { state: { kept: true } } },
      ]);
    },
  );
