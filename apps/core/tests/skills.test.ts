...
Use the approved parser.`;
    ...
    expect(skillInstructionsFromMarkdown(markdown)).toBe(
      "# PDF Processing\n\nUse the approved parser.",
    );
  });

  it("rejects invalid skill metadata", () => {
    expect(() => parseSkillMarkdown("missing")).toThrow(
...
---`),
  ...
  it("formats and parses account-scoped skill paths", () => {
    expect(formatSkillPath("acct_123", "support-flow")).toBe(
      "acct_123/support-flow",
    );
  ...
  it("sanitizes GitHub skill tree URLs", () => {
    ...
    expect(() =>
      parseGitHubSkillUrl("http://github.com/anthropics/skills/tree/main/pdf"),
    ).toThrow("GitHub skill URL must use https://github.com");
    expect(() =>
      parseGitHubSkillUrl("https://github.com/anthropics/skills/blob/main/pdf"),
    ).toThrow(
      "GitHub skill URL must be https://github.com/{owner}/{repo}/tree/{ref}/{path}",
    );
    expect(() =>
      parseGitHubSkillUrl(
        "https://github.com/anthropics/skills/tree/main/../secret",
      ),
...
describe("validateSkillBundle", () => {
  it("returns parsed metadata and normalized files without mutating the input", () => {
    const input = [
      bundleFile(
        "  SKILL.md  ",
        skillMarkdown("bundle-skill", "Bundle metadata."),
      ),
  ...
  it("throws when the bundle has no SKILL.md at the root", () => {
    expect(() =>
      validateSkillBundle([
        bundleFile("docs/SKILL.md", skillMarkdown("nested")),
      ]),
  ...
  it("throws on unsupported or binary files", () => {
    ...
    expect(() =>
      validateSkillBundle([skill, bundleFile("data.bin", "x")]),
    ).toThrow("Skill file must be a supported text file: data.bin");
    expect(() =>
      validateSkillBundle([
        skill,
        bundleFile("notes.txt", new Uint8Array([0x41, 0x00, 0x42])),
      ]),
  ...
  it("throws when a single file exceeds the per-file limit", () => {
    expect(() =>
      validateSkillBundle([
        bundleFile("SKILL.md", "x".repeat(5 * 1024 * 1024 + 1)),
      ]),
...
describe("contentTypeForSkillPath", () => {
  it("returns application/json for JSON files and text/plain otherwise", () => {
    ...
    expect(contentTypeForSkillPath("SKILL.md")).toBe(
      "text/plain; charset=utf-8",
    );
    expect(contentTypeForSkillPath("scripts/run.sh")).toBe(
      "text/plain; charset=utf-8",
    );
  });
});
