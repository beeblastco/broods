...
describe("sandbox config", () => {
  it("accepts a predefined size and rejects an unknown one", () => {
    expect(
      normalizeSandboxConfig({ provider: "sandbox", size: "medium" }).size,
    ).toBe("medium");
    expect(
      normalizeSandboxConfig({ provider: "sandbox" }).size,
    ).toBeUndefined();
    expect(() =>
      normalizeSandboxConfig({ provider: "sandbox", size: "huge" }),
    ).toThrow("config.size must be one of");
  });

  it("accepts a snapshot/image pin and rejects a non-string one", () => {
    expect(
      normalizeSandboxConfig({ provider: "sandbox", snapshot: "img_curated" })
        .snapshot,
    ).toBe("img_curated");
    expect(
      normalizeSandboxConfig({ provider: "sandbox" }).snapshot,
    ).toBeUndefined();
    expect(
      normalizeSandboxConfig({ provider: "sandbox", snapshot: "  " }).snapshot,
    ).toBeUndefined();
    expect(() =>
      normalizeSandboxConfig({ provider: "sandbox", snapshot: 7 }),
...
describe("sandbox config defaults & validation", () => {
  ...
  it("rejects unknown providers, permission modes, and runtimes", () => {
    ...
    expect(() => normalizeSandboxConfig({ permissionMode: "auto" })).toThrow(
      "config.permissionMode must be one of",
    );
    expect(() =>
      normalizeSandboxConfig({ runtimes: ["bash", "rust"] }),
    ).toThrow("config.runtimes must be a non-empty array");
    expect(() => normalizeSandboxConfig({ runtimes: [] })).toThrow(
      "config.runtimes must be a non-empty array",
    );
  });

  it("rejects non-string env vars, non-object options, and the removed internet field", () => {
    expect(() => normalizeSandboxConfig({ envVars: { OK: 1 } })).toThrow(
      "config.envVars must be an object with string values",
    );
    expect(() => normalizeSandboxConfig({ options: "nope" })).toThrow(
      "config.options must be an object",
    );
    expect(() => normalizeSandboxConfig({ internet: true })).toThrow(
      "config.internet is no longer supported",
    );
  });

  it("defaults network to deny-all and validates restricted allowlists", () => {
    expect(normalizeSandboxConfig({ provider: "lambda" }).network).toEqual({
      mode: "deny-all",
    });
    expect(
      normalizeSandboxConfig({
    ...
    expect(() =>
      normalizeSandboxConfig({
        provider: "lambda",
        network: { mode: "allow-all", allowDomains: ["api.example.com"] },
      }),
  ...
  it("rejects e2b configs that do not explicitly allow all network egress", () => {
    expect(() => normalizeSandboxConfig({ provider: "e2b" })).toThrow(
      "e2b cannot enforce egress restrictions",
    );
    expect(
      normalizeSandboxConfig({
        provider: "e2b",
...
describe("sandbox config persistent / lifecycle", () => {
  it("accepts persistent on the lambda (MicroVM) provider", () => {
    expect(
      normalizeSandboxConfig({ provider: "lambda", persistent: true })
        .persistent,
    ).toBe(true);
  });
  ...
  it("accepts persistent on sandbox/daytona/e2b/vercel", () => {
    expect(
      normalizeSandboxConfig({ provider: "sandbox", persistent: true })
        .persistent,
    ).toBe(true);
    expect(
      normalizeSandboxConfig({ provider: "daytona", persistent: true })
        .persistent,
    ).toBe(true);
    expect(
      normalizeSandboxConfig({
    ...
    expect(
      normalizeSandboxConfig({ provider: "vercel", persistent: true })
        .persistent,
    ).toBe(true);
  });
  ...
  it("requires persistent when lifecycle is set, and bounds its intervals", () => {
    ...
    ).toEqual({ idleTimeoutSeconds: 1800, maxLifetimeSeconds: 3600 });
    expect(() =>
      normalizeSandboxConfig({
        provider: "sandbox",
        persistent: true,
  ...
  it("requires persistent when lifecycle hooks are set", () => {
    ...
    expect(() =>
      ...
      "config.onCreate and config.onResume are not supported by the e2b provider",
    );
  });
});

describe("sandbox config update merge", () => {
  const existing: SandboxConfig = {
    provider: "lambda",
    permissionMode: "ask",
    network: { mode: "deny-all" },
    envVars: { A: "1" },
