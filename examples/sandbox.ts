    ...
    events: [
      {
        ...
        content: [{
          ...
          text: [
            ...
            "",
            "Env var check (the account configured SANDBOX_SMOKE_VAR=sandbox-env-ok):",
            "1. Run the shell command: echo \"shell:$SANDBOX_SMOKE_VAR\".",
            "2. Write /env.py that prints `python:` followed by os.environ['SANDBOX_SMOKE_VAR'], then run python3 /env.py.",
            "3. Write /env.js that prints `node:` followed by process.env.SANDBOX_SMOKE_VAR, then run node /env.js.",
            ...
            "8. Run node /weather.js.",
            "9. Return the stdout and status objects from every run.",
          ].join("\n"),
        }],
      },
    ],
