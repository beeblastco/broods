...
| `awsCredentialsVisible` | must be `false` — the runner scrubs its env before spawning the child |
| `tmpEntries` | the tool's own bundle is never written to disk, so it is not in here |
