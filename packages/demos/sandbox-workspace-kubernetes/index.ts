...
async function streamAgent(label: string, agentRef: ReturnType<typeof api.agents.analysisAgent>): Promise<void> {
  console.log(`\n\n=== ${label} ===`);

  for await (const chunk of client.stream(agentRef, {
    input: [
...
async function streamVisualizationAgent(label: string, agentRef: ReturnType<typeof api.agents.visualizationAgent>): Promise<void> {
  console.log(`\n\n=== ${label} ===`);
...
await streamAgent("Analysis agent stream", api.agents.analysisAgent);
await streamVisualizationAgent("Visualization agent stream", api.agents.visualizationAgent);
