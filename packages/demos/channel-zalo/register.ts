...
if (!botToken || !webhookSecret || !url) throw new Error("ZALO_BOT_TOKEN, ZALO_WEBHOOK_SECRET, and CHANNEL_WEBHOOK_URL are required");
...
if (!response.ok) throw new Error(`Zalo setWebhook failed: ${response.status} ${body}`);
