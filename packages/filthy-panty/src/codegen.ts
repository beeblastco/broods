...
export async function writeGeneratedFiles(
  ...
  await writeFile(resolve(dir, "api.ts"), apiFile(manifest, ids), "utf8");
...
function apiFile(manifest: CliManifest, ids: GeneratedIds): string {
  const agents = Object.keys(ids.agents);
  const agentEntries = agents.map((name) =>
    `    ${JSON.stringify(name)}: { kind: "agent", name: ${JSON.stringify(name)}, id: ids.agents[${JSON.stringify(name)}], project: ${JSON.stringify(manifest.project)}, environment: ${JSON.stringify(manifest.environment)} },`,
  ).join("\n");
