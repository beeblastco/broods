...
test("sends bearer auth and strips the trailing slash from baseUrl", async () => {
  const { client, calls } = mockClient([{ status: 200, body: { agents: [] } }]);
...
test("createAgent posts JSON and unwraps the created record", async () => {
  const { client, calls } = mockClient([
    {
      status: 201,
      body: { accountId: "acc_1", agentId: "agent_1", name: "tenant-a" },
    },
  ]);
...
test("account env vars: list, set, and delete use write-only config-plane routes", async () => {
  ...
  expect(await client.listEnvVars()).toEqual([
    { name: "OVH_API_KEY", updatedAt: 123 },
  ]);
...
test("unwraps list envelopes for crons, runs, workspaces, and files", async () => {
  const { client, calls } = mockClient([
    ...
    { status: 200, body: { workspaces: [{ workspaceId: "ws_1" }] } },
    {
      status: 200,
      body: {
        files: [{ path: "memory/notes.md", name: "notes.md", isFolder: false }],
      },
    },
  ]);

  expect((await client.listCrons())[0]?.cronId).toBe("cron_1");
  expect((await client.listCronRuns("cron_1", { limit: 5 }))[0]?.runId).toBe(
    "run_1",
  );
  ...
  expect(calls[1]?.url).toBe(
    "https://gateway.example.com/v1/crons/cron_1/runs?limit=5",
  );
});
  ...
  const { client } = mockClient([
    {
      status: 200,
      body: {
        account: { accountId: "acc_1", username: "beeblast", status: "active" },
      },
    },
  ]);
...
test("constructor requires accountSecret when env vars are absent", () => {
  ...
  try {
    ...
    if (savedSecret !== undefined)
      process.env.BROODS_ACCOUNT_SECRET = savedSecret;
  }
});
...
test("account metadata: update unwraps, delete returns cleanup, rotate returns the new secret", async () => {
  const { client, calls } = mockClient([
    ...
    {
      status: 200,
      body: { deleted: true, cleanup: { agentsDeleted: 2, cronsDeleted: 1 } },
    },
    {
      status: 200,
      body: {
...
test("sandboxes: list/create unwrap, get returns null on 404", async () => {
  const { client, calls } = mockClient([
    {
      status: 200,
      body: { sandboxes: [{ sandboxId: "sb_1", name: "default" }] },
    },
...
test("sandbox lifecycle: suspend posts the reservationKey and returns status", async () => {
  const { client, calls } = mockClient([
    { status: 200, body: { status: "suspended" } },
    {
      ...
      body: {
        token: "tkt_1",
        expiresAt: 123,
        websocketPath: "/v1/sandboxes/terminal/ws",
      },
    },
  ]);
  ...
  expect(calls[0]?.url).toBe(
    "https://gateway.example.com/v1/sandboxes/sb_1/suspend",
  );
  expect(JSON.parse(calls[0]?.body ?? "{}")).toEqual({
  ...
  expect(calls[1]?.url).toBe(
    "https://gateway.example.com/v1/sandboxes/sb_1/terminal",
  );
});
...
test("sandbox lifecycle throws on a missing sandbox (404)", async () => {
  ...
  expect(client.terminateSandbox("sb_missing", "key")).rejects.toThrow(
    BroodsAccountApiError,
  );
});
  ...
  expect(
    (
      await client.createPolicy({
        name: "guard2",
        document: { version: 1, rules: [] } as never,
      })
    ).policyId,
  ).toBe("pol_2");
...
test("skills: list unwraps, upload uses PUT, delete returns the flag", async () => {
  const { client, calls } = mockClient([
    {
      status: 200,
      body: {
        skills: [{ path: "acc_1/flow", name: "flow", description: "d" }],
      },
    },
    {
      status: 200,
      body: { path: "acc_1/flow", name: "flow", description: "updated" },
    },
    { status: 200, body: { deleted: true } },
  ]);
  ...
  const uploaded = await client.uploadSkill("flow", {
    source: "json",
    name: "flow",
    description: "updated",
    content: "# Flow",
...
test("workspace files: upload unwraps the file, rename and delete return flags", async () => {
  ...
  const file = await client.uploadWorkspaceFile("ws_1", {
  ...
  expect(calls[3]?.url).toBe(
    "https://gateway.example.com/v1/workspaces/ws_1/files?path=memory%2Frenamed.md",
  );
});
