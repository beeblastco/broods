...
The `broods` CLI does the interaction: sign-in, project sync, env, logs, running agents. The docs at https://docs.broods.app are the information: read the relevant page before guessing a config shape or an API route. The `broods` npm package is the SDK: it integrates a deployed agent into existing application code.
