...
async function handleChannelRecordRoute(
  ...
  if (!channelId) {
    ...
    if (req.method === "POST") {
      ...
      const createdId: Id<"channelRecords"> = await ctx.runMutation(
        internal.channelRecords.create,
        {
          accountId: accountId,
      ...
      const created: Doc<"channelRecords"> | null = await ctx.runQuery(
        internal.channelRecords.getById,
  ...
  if (req.method === "PATCH") {
    ...
    await ctx.runMutation(internal.channelRecords.update, {
      accountId: accountId,
  ...
  if (req.method === "DELETE") {
    ...
    if (!existing) return json({ error: "Channel not found" }, 404);
    await ctx.runMutation(internal.channelRecords.remove, {
      accountId: accountId,
      channelRecordId: channelId,
    });
    await writeAudit(ctx, {
