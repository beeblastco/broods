...
function fetchStub(responses: Array<() => Response | Error>): { impl: typeof fetch; urls: string[] } {
    const urls: string[] = [];
    let call = 0;
...
describe("slack channel directory", () => {
    ...
    it("maps HTTP 429 to a ratelimited result", async () => {
        ...
        const result = await fetchSlackChannelDirectory("xoxb-test", impl);
        expect(result).toMatchObject({ ok: false, status: 429, reason: "ratelimited" });
    });
    ...
    it("maps Slack auth and scope errors to their reasons", async () => {
        for (const [slackError, reason] of [
            ...
            const { impl } = fetchStub([() => slackJson({ ok: false, error: slackError })]);
            const result = await fetchSlackChannelDirectory("xoxb-test", impl);
    ...
    it("maps a thrown fetch (timeout / network) to a 502 slack_error", async () => {
        ...
        const result = await fetchSlackChannelDirectory("xoxb-test", impl);
        expect(result).toMatchObject({ ok: false, status: 502, reason: "slack_error" });
    });
