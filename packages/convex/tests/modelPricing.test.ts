...
describe("model pricing", () => {
    it("normalizes SDK providers and resolves gateway model identifiers", () => {
        ...
        expect(resolveModelTokenRates("gateway", "openai/gpt-5.4")?.input).toBe(2.5);
        expect(resolveModelTokenRates("bedrock", "anthropic.claude-sonnet-4-5-20250929-v1:0")?.cacheWrite).toBe(3.75);
    });
