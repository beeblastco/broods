...
export type SlackDirectoryEntry = { id: string; name: string; isPrivate: boolean; isMember: boolean };
...
export type SlackDirectoryResult =
    ...
    | { ok: false; status: number; error: string; reason: "ratelimited" | "missing_scope" | "invalid_auth" | "slack_error" };
...
export async function fetchSlackChannelDirectory(
    ...
    for (let page = 0; page < SLACK_DIRECTORY_PAGE_CAP; page++) {
        ...
        try {
            ...
            return { ok: false, status: 502, error: "Slack did not respond in time", reason: "slack_error" };
        }
        if (response.status === 429) {

            return { ok: false, status: 429, error: "Slack rate limit hit; retry shortly", reason: "ratelimited" };
        }
        ...
        if (data.ok !== true) {
            ...
            if (error === "missing_scope") {
                ...
                return { ok: false, status: 502, error: "The Slack app is missing a scope required to list public/private channels", reason: "missing_scope" };
            }
            if (error === "invalid_auth" || error === "not_authed" || error === "account_inactive" || error === "token_revoked") {

                return { ok: false, status: 502, error: "Slack rejected the stored bot token", reason: "invalid_auth" };
            }

            return { ok: false, status: 502, error: `Slack error: ${error}`, reason: "slack_error" };
        }
