...
export const provision = action({
  ...
  handler: async (
    ...
    const org = await ctx.runQuery(api.org.getByIdForAdmin, {
      orgId: args.orgId,
    });
    if (!org) {
    ...
    const existing = await ctx.runQuery(internal.accounts.getByOrgId, {
      orgId: args.orgId,
    });
    if (existing) {
      throw new Error(
...
export const rotateSecret = action({
  ...
  handler: async (ctx, args) => {
    ...
    const account = await ctx.runQuery(internal.accounts.getByOrgId, {
      orgId: args.orgId,
    });
    if (!account) {
