...
export const recordFailure = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!existing || existing.blockedUntil || args.now - existing.windowStart >= args.windowMs) {
            if (existing) {
        ...
        const blockedUntil = nextCount >= args.maxFailures ? args.now + args.blockMs : undefined;
        await ctx.db.patch(existing._id, {
            count: nextCount,
