...
export function environmentKindForName(
  environment: Pick<Doc<"environments">, "name" | "kind">,
): EnvironmentKind {
...
function findEnvironmentByKind(
  ...
  return environments.find(
    (environment) => environmentKindForName(environment) === kind,
  );
}
...
export const create = mutation({
  ...
  handler: async (ctx, { projectId, name, duplicateFromId }) => {
    ...
    if (duplicateFromId) {
      const source = await getOwnedEnvironment(
        ctx,
        authUser.id,
        duplicateFromId,
      );
...
export const initializeProduction = mutation({
  ...
  handler: async (
    ctx,
    { projectId, sourceEnvironmentId, deploymentRegion },
  ) => {
    const authUser = await authKit.getAuthUser(ctx);
    if (!authUser) throw new Error("User not found or not authenticated");
    ...
    for (const environment of existing.filter(
      (entry) =>
        entry._id !== productionId &&
        entry.isDefault &&
        environmentKindForName(entry) !== "development",
    )) {
