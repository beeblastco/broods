...
export const run = internalAction({
  ...
  handler: async (ctx, args) => {
    const accountIds: Id<"accounts">[] = await ctx.runQuery(
      internal.workosUserDeletion.listSoleOwnedAccounts,
      {
        authId: args.authId,
      },
    );
    ...
    if (!url || !secret) {
      ...
      for (const accountId of accountIds) {
        try {
          ...
          cleanupSucceeded = false;
          console.error(
            `WorkOS user deletion core cleanup failed for account ${accountId}`,
            error,
          );
        }
      }
    ...
    if (!cleanupSucceeded) {
      const delayMs: number | null = await ctx.runMutation(
        internal.workosUserDeletion.scheduleRetry,
        {
          authId: args.authId,
        },
      );
      if (delayMs !== null) {
        await ctx.scheduler.runAfter(
          delayMs,
          internal.workosUserDeletionCleanup.run,
          {
            authId: args.authId,
          },
        );
