...
export const list = internalQuery({
    ...
    handler: async (ctx, args) => {
        const rows = await ctx.db
            ...
            .withIndex("by_accountId_and_name", (q) => q.eq("accountId", args.accountId))
            .collect();
...
async function refreshSourceBackedAgents(
    ...
    for (const agent of agents) {
        if (!agent.encryptedSourceConfig || !agent.sourceEncryptionIv || !agent.sourceEncryptionTag) continue;
        ...
        if (!source) continue;
        const encrypted = await encryptAgentConfigBlob(substituteAccountEnvPlaceholders(source, values), secret);
