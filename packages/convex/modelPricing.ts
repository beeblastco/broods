...
const OPENAI_RATES: ReadonlyArray<[RegExp, ModelTokenRates]> = [
    ...
    [/^gpt-5-mini(?:-|$)/, { input: 0.25, cacheRead: 0.025, cacheWrite: 0.25, output: 2 }],
    [/^gpt-4\.1-mini(?:-|$)/, { input: 0.4, cacheRead: 0.1, cacheWrite: 0.4, output: 1.6 }],
];

const ANTHROPIC_RATES: ReadonlyArray<[RegExp, ModelTokenRates]> = [
    [/claude-(?:sonnet-4(?:-|$)|sonnet-4-[56])/, { input: 3, cacheRead: 0.3, cacheWrite: 3.75, output: 15 }],
    [/claude-haiku-4-5/, { input: 1, cacheRead: 0.1, cacheWrite: 1.25, output: 5 }],
    [/claude-opus-4-[56]/, { input: 5, cacheRead: 0.5, cacheWrite: 6.25, output: 25 }],
];

const GOOGLE_RATES: ReadonlyArray<[RegExp, ModelTokenRates]> = [
    [/^gemini-2\.5-flash-lite(?:-|$)/, { input: 0.1, cacheRead: 0.01, cacheWrite: 0.1, output: 0.4 }],
    [/^gemini-2\.5-flash(?:-|$)/, { input: 0.3, cacheRead: 0.03, cacheWrite: 0.3, output: 2.5 }],
];

const BEDROCK_RATES: ReadonlyArray<[RegExp, ModelTokenRates]> = [
    ...ANTHROPIC_RATES,
    [/amazon\.nova-lite-v1(?::0)?$/, { input: 0.06, cacheRead: 0.015, cacheWrite: 0.075, output: 0.24 }],
];
...
export function resolveModelTokenRates(provider: string, modelId: string): ModelTokenRates | null {
    ...
    if (normalizedProvider === "gateway" && normalizedModel.includes("/")) {
        ...
        normalizedProvider = canonicalModelProvider(normalizedModel.slice(0, separator));
        normalizedModel = normalizedModel.slice(separator + 1);
    }

    const tables: Readonly<Record<string, ReadonlyArray<[RegExp, ModelTokenRates]>>> = {
        openai: OPENAI_RATES,
        anthropic: ANTHROPIC_RATES,
        google: GOOGLE_RATES,
        bedrock: BEDROCK_RATES,
    };
    const match = tables[normalizedProvider]?.find(([pattern]) => pattern.test(normalizedModel));
