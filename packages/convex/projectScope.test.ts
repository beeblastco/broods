...
describe("agentsInProject", () => {
  test("returns the project's agents across every environment", async () => {
    ...
    const found = await tt.run((ctx) =>
      agentsInProject(ctx, s.projectA, s.accountId),
    );
    expect(found.map((a) => a._id).sort()).toEqual(
      [s.agentA1, s.agentA2].sort(),
    );
  });
...
describe("cronsInProject", () => {
  test("derives a cron's project from the agent it runs", async () => {
    ...
    const found = await tt.run((ctx) =>
      cronsInProject(ctx, s.projectA, s.accountId),
    );
  ...
  test("hides crons whose agent belongs to no project", async () => {
    ...
    const a = await tt.run((ctx) =>
      cronsInProject(ctx, s.projectA, s.accountId),
    );
    const b = await tt.run((ctx) =>
      cronsInProject(ctx, s.projectB, s.accountId),
    );
