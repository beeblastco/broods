...
export const create = internalAction({
  ...
  handler: async (ctx, args): Promise<Record<string, unknown>> => {
    ...
    const cronId: Id<"crons"> = await ctx.runMutation(
      internal.agent.crons.create,
      {
    ...
    try {
      ...
      await ctx
        .runMutation(internal.agent.crons.remove, {
          accountId: args.accountId,
          cronId: cronId,
        })
        .catch(() => {});
...
async function deleteCronEverywhere(
  ...
  let deleted = 0;
  do {
    deleted = await ctx.runMutation(internal.agent.crons.removeRuns, {
...
async function getOwnedCron(
  ...
  try {
    return await ctx.runQuery(internal.agent.crons.getById, {
      accountId: accountId,
...
async function requireAccountAgent(
  ...
  const agent = await ctx
    .runQuery(internal.agent.agents.getById, {
      accountId: accountId,
