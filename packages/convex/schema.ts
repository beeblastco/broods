...
/** Detached async tool state, including delivery and callback authorization. */
export const runtimeAsyncToolResultsFields = { accountId: v.string(), resultId: v.string(), parentEventId: v.string(), conversationKey: v.string(), toolName: v.string(), toolCallId: v.string(), input: v.any(), status: v.union(v.literal("processing"), v.literal("completed"), v.literal("failed")), response: v.optional(v.any()), error: v.optional(v.string()), delivery: v.optional(v.any()), completionToken: v.optional(v.string()), observed: v.optional(v.boolean()), createdAt: v.string(), updatedAt: v.string(), expiresAt: v.number() };
