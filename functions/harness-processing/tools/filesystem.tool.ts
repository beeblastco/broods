...
- python: prefer running it as a standalone command — \`python3 <name>.py\` or \`python <name>.py\` — so it uses the dedicated native CPython runtime (best performance and full stdlib). Python invoked inside a larger shell command runs on a slower, limited in-process WASM Python and may fail on complex scripts.`;
}
