...
5. **Checks.** Run the `AGENTS.md` before-done commands and tests for each touched workspace. For lines the diff adds: `bunx oxlint --type-aware -A all -D typescript/no-explicit-any -D typescript/no-unsafe-type-assertion <files>`. The OpenAPI spec changed: `oasdiff breaking <base>:apps/docs/docs/api-reference/openapi.yaml apps/docs/docs/api-reference/openapi.yaml --fail-on ERR`, with `<base>` the fetched PR target. `packages/broods` changed: `pack:check`. Say what you skipped and why.
