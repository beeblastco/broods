...
Model of how a manifest sync treats crons. The client pins `config.name` to the
resource name (`normalizeConfig` in `packages/broods/src/manifest.ts`). The main
sync prunes undeclared agents and deletes their crons with them (`deleteOwnedAgent`
in `packages/convex/model/cliSyncResources.ts`). `syncCrons`
(`packages/convex/cli/httpRoutes.ts`) then upserts by name among the stage agents'
crons and, with prune, drops the stage agents' undeclared crons.
-/
...
/-- `normalizeConfig` for a cron: the name the server keys it by is the resource
name, whatever `config.name` said. -/
def cronName (resourceName : Nat) (_configName : Option Nat) : Nat := resourceName
...
/-- The diff and the server key a cron by the same name. -/
theorem cron_key_agrees (resourceName : Nat) (configName : Option Nat) :
    cronName resourceName configName = resourceName := rfl
...
/-- A stray `config.name` no longer splits the key: the cron is keyed `1`, not `9`. -/
example : cronName 1 (some 9) = 1 := rfl
