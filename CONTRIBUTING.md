...
- **New tool.** Create `apps/core/src/harness/tools/<name>.tool.ts`, export a default factory, register it in `apps/core/src/harness/tools/index.ts`, and add config validation in `packages/convex/model/agentRules.ts`.
